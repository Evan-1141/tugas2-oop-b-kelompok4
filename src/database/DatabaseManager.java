    ...
    private static void seedData(Connection conn) throws SQLException {
        ...
        stmt.execute(
                "INSERT OR IGNORE INTO venues "
                        ...
                        + "VALUES "
                        + "('VEN-001','BNDCC','Nusa Dua, Bali',5000)");

        stmt.execute(
                "INSERT OR IGNORE INTO venues "
                        + "(id, name, address, max_capacity) "
                        + "VALUES "
                        + "('VEN-002','Auditorium Udayana','Jimbaran, Bali',1000)");

        stmt.execute(
                "INSERT OR IGNORE INTO venues "
                        + "(id, name, address, max_capacity) "
                        + "VALUES "
                        + "('VEN-003','Lapangan Renon','Denpasar, Bali',10000)");
        ...
        stmt.execute(
                "INSERT OR IGNORE INTO events "
                        ...
                        + "VALUES "
                        + "('EVT-001','concert','Coldplay Music of The Spheres','VEN-001','USR-001','2026-08-20',1500000)");

        stmt.execute(
                "INSERT OR IGNORE INTO events "
                        + "(id, type, name, venue_id, organizer_id, date, base_price) "
                        + "VALUES "
                        + "('EVT-002','seminar','AI Future Summit 2026','VEN-002','USR-002','2026-09-10',250000)");

        stmt.execute(
                "INSERT OR IGNORE INTO events "
                        + "(id, type, name, venue_id, organizer_id, date, base_price) "
                        + "VALUES "
                        + "('EVT-003','sport_match','Bali United vs Persija','VEN-003','USR-001','2026-10-15',100000)");
